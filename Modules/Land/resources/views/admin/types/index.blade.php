...
@section('toolbar')
    ...
    @endphp
    <x-admin.breadcrumb :pageTitle="__('Lands Types')" :breadcrumbItems="$breadcrumbItems"/>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3 class="modal-title">{{__('Add New Land Type')}}</h3>
                    ...
                    <form method="POST" action="{{route('admin.lands.types.store')}}">
                        @csrf
                        ...
                        <div class="modal-body">
                            <div class="mb-5">
                                ...
                                <input type="text" class="form-control form-control-solid" name="name" id="name"
                                       value="{{old('name')}}"
                                       placeholder="example: Agricultural"/>
                            </div>
                        </div>
...
@section('content')
    <x-admin.table :model="$types" search="Search In Lands Types"
                   :formUrl="route('admin.lands.types.deleteMulti')">
        <!--begin::Table head-->
        <thead>
...
@section('js')
    <script>
        $(document).ready(function (e) {
            ...
            function resetForm() {
                $('#method').val('POST');
                $('form').attr('action', "{{ route('admin.lands.types.store') }}");
                $('#name').val('');
            }
            ...
            $('#addNewBtn').on('click', function () {
                resetForm();
                $('.modal-title').text("{{__('Add New Land Type')}}");
            ...
            $('.editBtn').on('click', function () {
                ...
                $('.modal-title').text("{{__('Edit Land Type')}}");

                var updateUrl = "{{ route('admin.lands.types.update', ':type') }}";
